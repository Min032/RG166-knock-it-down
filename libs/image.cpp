...
auto
image_init (int width, int height) -> Image *
{
	...
	if (width == 0 || height == 0)
		...
		{
			image->pixels
					= static_cast<char *> (malloc (3 * width * height * sizeof (char)));
...
void
image_read (Image *image, char *filename)
{
	...
	if (bih.bitcount == 24)
		{
			image->pixels = static_cast<char *> (
					malloc (3 * bih.width * bih.height * sizeof (char)));
		}
	else if (bih.bitcount == 32)
		{
			image->pixels = static_cast<char *> (
					malloc (4 * bih.width * bih.height * sizeof (char)));
		}
	else
		{
			fprintf (stderr,
